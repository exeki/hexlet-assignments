...
public class ProductsController {
    ...
    private Product updateEntity(ProductUpdateDTO productData, Product product) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ProductDTO> updateProduct(@PathVariable Long id, @RequestBody ProductUpdateDTO productData){
        var product = productRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Product with id " + id + " not found"));
        var updated = productRepository.save(updateEntity(productData, product));
